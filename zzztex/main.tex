...
\paragraph{DR1: Ground interpretation in active analytical commitments.}

The system should interpret a new utterance relative to the current analytical
goal, working hypothesis, filters, visualizations, and dialogue context. This
context is necessary to determine whether the utterance continues, qualifies,
or supersedes the active investigation.

\paragraph{DR2: Support compound revisions through composable actions.}

Because one utterance may revise multiple analytical objects, the system should
support composable, schema-grounded action sequences rather than map every
utterance to one operation or one exclusive revision type.

\paragraph{DR3: Distinguish analytical revision from repair and method change.}

Recognition corrections, clarifications, evidence requests, initial
hypotheses, and changes in chart or metric should not automatically invalidate
the current analytical direction. The system should preserve valid context
when the user is repairing expression rather than revising analysis.

\paragraph{DR4: Coordinate supersession across speech, execution, and visual
state.}

When a new request supersedes an active response, cancellation should extend
beyond audio playback. Response-dependent tool calls, query execution, and
pending dashboard updates should be invalidated so that obsolete results
cannot later modify the visible state. Replanning should begin from the latest
committed dashboard state while preserving still-relevant filters,
visualizations, findings, and dialogue context.

These requirements directly informed VerbalVis: compact dashboard-state
grounding supports DR1; composable visualization tools support DR2; separate
handling of repair and analytical redirection supports DR3; and response-bound
execution epochs with stale-result rejection support DR4.
