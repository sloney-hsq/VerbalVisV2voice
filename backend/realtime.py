...
POST_INSTRUCTIONS_TOKEN_LIMIT = int(os.getenv("OPENAI_REALTIME_POST_INSTRUCTIONS_LIMIT", "12000"))
TRUNCATION_RETENTION_RATIO = float(os.getenv("OPENAI_REALTIME_RETENTION_RATIO", "0.8"))
...
_legacy_ptt = os.getenv("VERBALVIS_PUSH_TO_TALK")
INPUT_MODE = os.getenv("VERBALVIS_INPUT_MODE")
if not INPUT_MODE:
    INPUT_MODE = (
        "push_to_talk"
        if _legacy_ptt and _legacy_ptt.lower() not in {"0", "false", "no", "off"}
        else ("open_mic" if _legacy_ptt else "local_vad")
    )
INPUT_MODE = INPUT_MODE.lower().replace("-", "_")
if INPUT_MODE not in {"local_vad", "push_to_talk", "open_mic"}:
    INPUT_MODE = "local_vad"
MANUAL_COMMIT_MODE = INPUT_MODE in {"local_vad", "push_to_talk"}
SERVER_VAD_TYPE = os.getenv("VERBALVIS_SERVER_VAD_TYPE", "server_vad")
if SERVER_VAD_TYPE not in {"server_vad", "semantic_vad"}:
    SERVER_VAD_TYPE = "server_vad"
...
class RealtimeSession:
    ...
    def _build_session_config(self, profile: str) -> dict[str, Any]:
        ...
        if MANUAL_COMMIT_MODE or not BARGE_IN_ENABLED:
            # Explicitly disable the model/server VAD defaults for local_vad and
            # push_to_talk. Omitting this can leave the default server_vad active,
            # which creates surprise speech_started/response.create events.
            audio_input["turn_detection"] = None
        elif profile != "minimal":
            if SERVER_VAD_TYPE == "semantic_vad":
        ...
        session: dict[str, Any] = {
            ...
            "truncation": {
                ...
                "token_limits": {
                    "post_instructions": POST_INSTRUCTIONS_TOKEN_LIMIT,
                },
            },
            "audio": {
                "input": audio_input,
        ...
        try:
            while self._running:
                ...
                if msg_type == "audio":
                    ...
                    self._pending_audio_ms = 0
                    if BARGE_IN_ENABLED:
                        await self._truncate_assistant_audio(msg.get("assistant_audio"))
                    ...
                    self._pending_audio_ms = 0
                    await self._create_response_if_idle("client.commit")
    ...
    async def _openai_to_client(self) -> None:
        ...
        async for raw in self.openai_ws:
            ...
            # Accept both for safety.
                ...
                # assistant message items out of tool_calls.log so repetitions
                # in speech are not mistaken for repeated tools.
    ...
    def _track_assistant_audio(self, event: dict[str, Any]) -> None:
        ...
        try:
            ...
            return
        # PCM16 mono at 24 kHz: 48 bytes ~= 1 ms.
        self._current_assistant_audio_generated_ms += round(byte_count / 48)
    ...
    async def _truncate_assistant_audio(self, assistant_audio: Any = None) -> None:
        ...
        self._record_timeline(
            "conversation.item.truncate.sent",
            item_id=item_id,
            content_index=content_index,
            audio_end_ms=audio_end_ms,
        )
        if self._bargein_logger:
            self._bargein_logger.info(
                "TRUNCATE item_id=%s content_index=%s audio_end_ms=%s",
    ...
    def _compact_tool_payload(self, result: dict[str, Any]) -> Any:
        """Return model-facing tool output without bulky chart data rows."""
        ...
        if usage:
            ...
            if self._event_logger:
                self._event_logger.info(
                    ...
                    response_id,
                    json.dumps(metrics["usage"], ensure_ascii=False),
                )
